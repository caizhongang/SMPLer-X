...
def _split_channels(num_chan, num_groups, split_op='equal'):
    if split_op == 'equal':
        # split = [num_chan // num_groups for _ in range(num_groups)]
        split = [(num_chan // num_groups) // 4 * 4 for _ in range(num_groups)]
        split[0] += num_chan - sum(split)
    elif split_op == 'exp':
        split = [int((num_chan * math.pow(2, -i))) // 4 * 4 for i in range(1, num_groups + 1)]
        split[0] += num_chan - sum(split)
    else:
        raise ValueError('Unknown split option: {}'.format(split_op))
    return split
...
class MixedConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
        ...
        in_splits = _split_channels(in_channels, num_groups, split_op=split_op)
        out_splits = _split_channels(out_channels, num_groups, split_op=split_op)
    ...
    def forward(self, x):
        if len(self.splits) > 1:
            ...
            x_out = [c(x) for c in self._modules.values()]
            x = x_out[0]
        return x
