...
if [[ ! -d "reid_eval_logs" ]]; then
  mkdir reid_eval_logs
fi
...
TEST_CONFIG=${4-reid_test_market.yaml}
GINFO_INDEX=${5-20}   # task index config cherrypick (if necessary)
...
LOG_FILE=reid_eval_logs/test_${job_name}.log
