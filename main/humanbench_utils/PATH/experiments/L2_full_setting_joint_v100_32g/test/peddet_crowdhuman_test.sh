...
if [[ ! -d "peddet_crowdhuman_eval_logs" ]]; then
  mkdir peddet_crowdhuman_eval_logs
fi
...
TEST_CONFIG=${4-peddet_crowdhuman_test.yaml}
GINFO_INDEX=${5-23}   # task index config cherrypick (if necessary)
...
LOG_FILE=peddet_crowdhuman_eval_logs/${job_name}.log
