...
if [[ ! -d "parsing_human36_eval_logs" ]]; then
  mkdir parsing_human36_eval_logs
fi
...
TEST_CONFIG=${4-parsing_test_human36.yaml}
GINFO_INDEX=${5-11}   # task index config cherrypick (if necessary)
...
LOG_FILE=parsing_human36_eval_logs/${job_name}.log
