...
if [[ ! -d "parsing_CIHP_eval_logs" ]]; then
  mkdir parsing_CIHP_eval_logs
fi
...
TEST_CONFIG=${4-parsing_test_CIHP.yaml}
GINFO_INDEX=${5-13}   # task index config cherrypick (if necessary)
...
LOG_FILE=parsing_CIHP_eval_logs/${job_name}.log
