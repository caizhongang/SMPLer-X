...
class RenBody(HumanDataset):
    def __init__(self, transform, data_split):
        ...
        if self.use_cache and osp.isfile(self.annot_path_cache):
            print(f'[{self.__class__.__name__}] loading cache from {}'.format(self.annot_path_cache))
            ...
            for idx in [5, 6, 7, 8, 9]:
                if self.data_split == 'train':
                    pre_prc_file_train = f'renbody_train_230517_399_{idx}.npz'
                    filename = getattr(cfg, 'filename', pre_prc_file_train)
                else:
                    if idx > 1: continue
                    pre_prc_file_test = f'renbody_test_230517_78_{idx}.npz'
