        ...
        for data_path in tqdm.tqdm(data_path_list):
            ...
            for i in tqdm.tqdm(range(img_num)):
                ...
                for j in range(person_num):
                    ...
                    try:
                        assert np.allclose(gt_joints_3d, new_joints, atol=1e-6)
                        assert np.allclose(gt_verts, new_vertices, atol=1e-6)
                    except:
                        print('error')
                        import pdb
                        pdb.set_trace()
...
def generate_new_json():
    ...
    for split in ('train', 'validation'):
        ...
        for ann in tqdm.tqdm(annotations):
            ...
            new_smplx_param_path = smplx_param_path.replace('smplx_gt', 'smplx_gt_fix_global_orient_transl')
